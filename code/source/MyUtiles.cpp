...
void Log(const char *fmt, ...)
{
	...
	{
		char	filename[512];
		_snprintf(filename, sizeof(filename), "%s\\%s", g_szWorkingDirectory, "ThermHunter.log");
	...
	if (g_flLogAll == NULL)
	{
		...
		_snprintf(filename_all, sizeof(filename_all), "%s\\%s", g_szWorkingDirectory, "ThermHunter_all.log");
